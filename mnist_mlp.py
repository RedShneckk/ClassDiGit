...
if __name__ == "__main__":
  ...
  for i in (t:=trange(getenv("STEPS", 70))):
    ...
    if (i % 10 == 9) and (test_acc := get_test_acc().item()) > best_acc:
      ...
      # nom unique pour éviter le verrouillage Windows
      best_ckpt_path = dir_name / f"{model_name}_best_{i+1:04d}.safetensors"
      safe_save(state_dict, best_ckpt_path)
      continue
    ...
    if (best_since := best_since + 1) % PATIENCE == PATIENCE - 1:
      ...
      opt.lr *= LR_DECAY
      if best_ckpt_path:
        state_dict = safe_load(best_ckpt_path)
